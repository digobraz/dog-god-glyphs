// ════════════════════════════════════════════════════════════════════════════
// WALL LAB — PAPYRUSOVÝ NAV (DEV ONLY, používajú ho iba *-lab stránky)
// ────────────────────────────────────────────────────────────────────────────
// 1:1 kópia `PageNav.tsx` z 25. 8. 2026, prefarbená na bledý (papyrusový) povrch.
// Zámerne SAMOSTATNÝ SÚBOR — `PageNav.tsx` je chrome ostrého webu a nesmie sa
// pri experimente pohnúť. Divergencia je ÚČEL, nie dlh; späť sa nekopíruje nič,
// kým Matej neodklepne celý svetlý režim.
//
// ⚠️ Odkazy v NAV_ITEMS mieria na LAB routy (/vision-lab, /religion-lab,
//    /about-lab), aby sa človek pri klikaní neprepadol späť do tmavého webu.
// ⚠️ Tmavý medailón loga (`#000` kruh) ostáva zámerne — logo je zlato-na-čiernom
//    asset a na papyruse funguje ako ohnisko, nie ako pozostatok tmavej témy.
// ════════════════════════════════════════════════════════════════════════════
...
const NAV_ITEMS = [
  { key: 'nav.wall', to: '/wall-lab' },
  { key: 'nav.vision', to: '/vision-lab' },
  { key: 'nav.religion', to: '/religion-lab' },
  { key: 'nav.about', to: '/about-lab' },
];
...
const LANGS: { code: LangCode; flag: string; label: string; native: string; enabled: boolean }[] = [
  ...
  { code: 'chn', flag: '🇨🇳', label: 'CHN', native: '中文',         enabled: true  },
  { code: 'jpn', flag: '🇯🇵', label: 'JPN', native: '日本語',        enabled: true  },
  { code: 'ind', flag: '🇮🇳', label: 'IND', native: 'हिन्दी',      enabled: true  },
  { code: 'ara', flag: '🇸🇦', label: 'ARA', native: 'العربية',    enabled: true  },
  { code: 'rus', flag: '🇷🇺', label: 'RUS', native: 'Русский',    enabled: true  },
  { code: 'kor', flag: '🇰🇷', label: 'KOR', native: '한국어',        enabled: true  },
  ...
  { code: 'nld', flag: '🇳🇱', label: 'NLD', native: 'Nederlands', enabled: true  },
  ...
  { code: 'tur', flag: '🇹🇷', label: 'TUR', native: 'Türkçe',     enabled: true  },
];
          ...
          >
            <Link
              ...
              style={{
                ...
                color: isActive ? '#C99A3F' : 'rgba(110,71,16,0.62)',
                textDecoration: 'none',
            ...
            <span
              ...
              style={{
                ...
                width: 1,
                height: 12,
                background: 'rgba(201,154,63,0.38)',
              }}
            />
          </span>
        );
      ...
      >
        <style>{`
          @keyframes chevronPulse {
            0%, 100% {
              filter: drop-shadow(0 0 0 rgba(201, 154, 63, 0));
              opacity: 0.88;
              transform: translateY(0);
            }
            50% {
              filter: drop-shadow(0 0 6px rgba(201, 154, 63, 0.60));
              opacity: 1;
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          inset: 0,
          background:
            'radial-gradient(ellipse at center, rgba(255,255,255,0.42) 0%, rgba(201,154,63,0.10) 62%, rgba(138,90,20,0.20) 100%)',
          pointerEvents: 'none',
        }}
      />
          ...
          return (
            <Link
              ...
              style={{
                ...
                textTransform: 'uppercase',
                color: isActive ? '#C99A3F' : 'rgba(58,34,4,0.80)',
                textDecoration: 'none',
                borderBottom: isActive
                  ? '1.5px solid #C99A3F'
