    ...
    <div className="min-h-[100dvh] relative" style={{ backgroundColor: T.pageBg, color: T.onDark }}>
      ...
      {DEV_FULL && (
        ...
        >
          ...
          >
            <FloatingNavLink to="/pack" label={t('pack.nav.home')} icon={iconHome} end />
            <FloatingNavLink to="/pack/portal" label={t('pack.nav.portal')} icon={iconPortal} />
          </div>
        </nav>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ ...glassPill, flex: '3 1 0', minWidth: 0, gap: 10, width: '100%' }}>
        <button type="button" onClick={onProfile} style={{ flexShrink: 0, lineHeight: 0 }} aria-label={t('pack.nav.profile')}>
          {avatarUrl ? (
            <img
              src={avatarUrl}
              alt={t('pack.nav.yourAvatar')}
      ...
      <div
        ...
        role="button"
        tabIndex={0}
        aria-label={t('pack.nav.jumpToNationStats')}
...
function BonesChip({ bones }: { bones: number }) {
  ...
  return (
    <div
      title="BONES"
      aria-label={t('pack.nav.bonesCount', { count: bones })}
    ...
    <div style={{
      ...
      {dogs.map((dog) => (
        <button
          ...
          aria-label={dog.dog_name || t('pack.nav.dog')}
        >
