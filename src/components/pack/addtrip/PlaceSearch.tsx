...
// `country` = ISO2 štátu z `regionalStructure` (malými). Doplnené 26. 9. 2026 pre PRIANIA:
// výlet v tom istom štáte splní prianie „štát" (hláška D1).
export type PlaceSug = { name: string; sub: string; lat: number; lon: number; type?: string; country?: string };
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const next: PlaceSug[] = (data.items || [])
          .map((it: { name?: string; label?: string; location?: string; type?: string; position?: { lat?: number; lon?: number }; regionalStructure?: { type?: string; isoCode?: string }[] }) => ({
            name: it.name || '',
            type: it.type,
            country: it.regionalStructure?.find((r) => r.type === 'regional.country')?.isoCode?.toLowerCase(),
