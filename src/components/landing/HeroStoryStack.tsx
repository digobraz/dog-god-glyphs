 ...
 * Stack height = 230dvh:
 *   - 100dvh sticky hero on top
 *   - 130dvh trailing scroll area where video rises and locks
 * Scroll progress 0 → 1 across the stack drives all animations.
 */
export function HeroStoryStack() {
  ...
  // Hero text: fade out + scale down across the first ~45% of scroll
  ...
  // Down arrow fades quickly on first scroll
  const arrowOpacity = useTransform(scrollYProgress, [0, 0.1], [1, 0]);

  // Video: rises from below, scales up, fades in from 25% → 65%
  const videoOpacity = useTransform(scrollYProgress, [0.25, 0.65], [0, 1]);
  const videoY = useTransform(scrollYProgress, [0.25, 0.65], [40, 0]);
  const videoScale = useTransform(scrollYProgress, [0.25, 0.65], [0.93, 1]);

  // Background "STORY" caption rises with video
  const captionOpacity = useTransform(scrollYProgress, [0.35, 0.7], [0, 1]);

  // Black background fades in as video takes over
  const bgFade = useTransform(scrollYProgress, [0.15, 0.5], [0, 1]);
  ...
  return (
    ...
    >
      ...
      >
        {/* Black overlay that fades in as we scroll past hero */}
        <motion.div
          className="absolute inset-0 bg-black pointer-events-none"
          style={{ opacity: bgFade, zIndex: 5 }}
        />
        ...
        {/* Hero content (fades + scales down on scroll) */}
        <motion.div
          className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 z-20 pt-[120px] md:pt-[140px]"
        ...
        {/* Pulsing arrow */}
        <motion.div
          className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 animate-pulse"
          style={{ color: '#C49B42', opacity: arrowOpacity }}
        >
          <ChevronDown className="w-8 h-8 md:w-10 md:h-10" />
        </motion.div>
      </div>

      {/* VIDEO — sticky from below the hero, slides up over scroll */}
      <div
        className="sticky w-full flex flex-col items-center justify-center px-6 md:px-10"
        style={{ top: '64px', height: 'calc(100dvh - 64px)', marginTop: '-100dvh', zIndex: 10 }}
      >
        <motion.div
