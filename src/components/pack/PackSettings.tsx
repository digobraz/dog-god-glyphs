...
export function PackSettings() {
  ...
  const handleSetPassword = async () => {
    setPwError('');
    if (pwValue.length < 8) { setPwError(t('pack.settings.error.minLength')); return; }
    if (pwValue !== pwConfirm) { setPwError(t('pack.settings.error.noMatch')); return; }
    setPwSaving(true);
    try {
      ...
      toast({ title: t('pack.settings.toast.title'), description: t('pack.settings.toast.description') });
    } catch (err) {
      setPwError(err instanceof Error ? err.message : t('pack.settings.error.generic'));
    } finally {
      setPwSaving(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        {t('pack.settings.section.account')}
      </div>

      <Field icon={<Mail className="h-4 w-4" />} label={t('pack.settings.field.email')}>
        <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 14, color: T.ink }}>
          {email}
        </span>
      </Field>
      <Field icon={<KeyRound className="h-4 w-4" />} label={t('pack.settings.field.password')}>
        <button
          type="button"
        ...
        >
          {t('pack.settings.password.setChange')}
        </button>
      </Field>
      <Field icon={<BrandIcon name="globe" size={16} tint="gold" />} label={t('pack.settings.field.language')}>
        <div className="flex items-center gap-2">
          <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 14, color: T.ink }}>English</span>
          <Badge label={t('pack.settings.badge.lockedV0')} />
        </div>
      </Field>
      <Field icon={<BellOff className="h-4 w-4" />} label={t('pack.settings.field.notifications')}>
        <Badge label={t('pack.settings.badge.comingSoon')} />
      </Field>
      <Field icon={<LogOut className="h-4 w-4" />} label={t('pack.settings.field.signOut')} last>
        <button
          type="button"
        ...
        >
          ...
          {t('pack.settings.signOut.button')}
        </button>
      </Field>
          ...
          >
            <div className="flex items-center justify-between" style={{ marginBottom: 6 }}>
              <span style={{ fontFamily: "'Cinzel', serif", fontSize: 11, letterSpacing: '0.32em', textTransform: 'uppercase', color: T.inkDim }}>
                {t('pack.settings.modal.title')}
              </span>
              <button
                type="button"
                onClick={() => setPwModalOpen(false)}
                aria-label={t('pack.settings.modal.close')}
            ...
            <p style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, color: T.inkDim, margin: '0 0 16px' }}>
              {fromWelcome
                ? t('pack.settings.modal.descWelcome')
                : t('pack.settings.modal.desc')}
            </p>
            <div className="flex flex-col gap-3">
              <input
                ...
                placeholder={t('pack.settings.modal.newPasswordPlaceholder')}
              ...
              <input
                ...
                placeholder={t('pack.settings.modal.confirmPasswordPlaceholder')}
                style={{ width: '100%', background: T.bg, border: `1px solid ${T.hairline}`, borderRadius: 10, padding: '12px 14px', color: T.ink, fontFamily: "'Space Grotesk', sans-serif", fontSize: 14, outline: 'none' }}
              />
              {pwError && (
